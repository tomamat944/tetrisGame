...
{
    class BlockBreakR: Block
    {
        public BlockBreakR(Tuple<int, int> p)
        {
            position_block = p;
            block = new Tile[2, 3]
            {
                ...
                {new Tile(Tuple.Create(p.Item1+1,p.Item2),Enums.Type_of_block.NONE),
                    new Tile(Tuple.Create(p.Item1+1,p.Item2+1),Enums.Type_of_block.BLOCK),
                    new Tile(Tuple.Create(p.Item1+1,p.Item2+2),Enums.Type_of_block.BLOCK) }
            };
        }
    }
}
